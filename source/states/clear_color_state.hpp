...
class ClearColorState: public our::State {
    ...
    void onInitialize() override {
        ...
        auto config = getApp()->getConfig()["scene"]["clear-color"];
        float clearColorChannels[4] =  {
            config.value<float>("r",0.0),
            config.value<float>("g",0.0),
            config.value<float>("b",0.0),
            config.value<float>("a",0.0) 
        };
        glClearColor(clearColorChannels[0],clearColorChannels[1],
                     clearColorChannels[2],clearColorChannels[3]);
    }
